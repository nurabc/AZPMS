...
if (!isset($_SESSION['comissioner_logged'])) {
  ...
  if (isset($_POST['sendddd'])) {
  $msgto=$_POST['messageto'];
  $sndto=$_POST['sendto'];
  $zone=$_POST['zone'];
  if ($sndto=="administrator") {
  $sql="INSERT INTO `admin_message`(`id`, `sender`, `message`, `zone`, `status`, `date`) VALUES ('','comissioner','".$msgto."','".$zonee."','0','".date("Y-m-d")."')";
  $query=mysqli_query($con,$sql);
  if ($query) {
    echo "<script>alert('sucessfull sent')</script>";
  }
  else{
  ...
  if ($sndto=="inspector") {
  $sql="INSERT INTO `inspector_message`(`id`, `sender`, `message`, `zone`, `status`, `date`) VALUES ('','comissioner','".$msgto."','".$zone."','0','".date("Y-m-d")."')";
  $query=mysqli_query($con,$sql);
  if ($query) {
    echo "<script>alert('sucessfull sent')</script>";
  }
  ...
  if ($sndto=="policeofficer") {
  $sql="INSERT INTO `policeofficer_message`(`id`, `sender`, `message`, `zone`, `status`, `date`) VALUES ('','comissioner','".$msgto."','".$zone."','0','".date("Y-m-d")."')";
  $query=mysqli_query($con,$sql);
  if ($query) {
    echo "<script>alert('sucessfull sent')</script>";
  }
