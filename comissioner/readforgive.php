...
<table class="table table-bordered ">
  ...
  <tr>
    ...
    <tr style="text-align: center;"><td style="font-size: 20px; background-color: LightSteelBlue; color: white; font-family: comic sans Ms;">Do you know about Assosa Zone</td>
        ...
        <tr><td class="text-justify" style="font-size: 14px;"> Assosa Zone Located in  Benshangul/Gumuz, is one of the nine ethnic divisions (kililoch) of Ethiopia. It was previously known as Region 6.The region's capital is Assosa<br>
            <img src="../tasari2.jfif" width="100%"></td> 
            <td style="width: 450px;" class="text-justify" rowspan="3">
              <?php 

$sql="SELECT * FROM `forgive` where status='0' and zone='".$zone."'";
$query=mysqli_query($con,$sql);
$report=mysqli_num_rows($query);
if ($report>0) {
echo "<table class='table table-bordered' style='font-size:20px;'><tr><td>date</td><td>serial_no</td><td>description</td><td colspan='2' class='text-center'>status</td>";
while ($row=mysqli_fetch_array($query)) {
echo "<tr><td>".$row['date']."</td> <td>".$row['serial_no']."</td> <td>".$row['description']."</td> 
<td><a href='approved.php?id=".$row['id']." '>approve</a></td>
<td><a href='disapproved.php?id=".$row['id']."'>disapprove</a></td>
</tr>";
}
echo "</table>";
}
else{
  echo "<font style='color:red; font-size:20px;'>untile now any request is not posted!</font>";
}
...
<div style="background-color: #E6E6FA;border-radius: 5px; box-shadow: 0px 0px 10px 0px green;"><center><font style="font-size: 30px;">Copyright © is Reserved ASU CS G-4 2024</font></center></div>
