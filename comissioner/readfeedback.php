...
$sql="SELECT * FROM `feedback` where status='0' and zone='".$zone."'";
$query=mysqli_query($con,$sql);
$report=mysqli_num_rows($query);
if ($report>0) {
echo "<table class='table table-bordered' style='font-size:20px;'><tr><td>sender</td><td>message</td><td>date</td><td>status</td>";
while ($row=mysqli_fetch_array($query)) {
echo "<tr><td>".$row['sender']."</td> <td>".$row['comment']."</td> <td>".$row['date']."</td> 
<td><a href='readfeed.php?id=".$row['id']."'>read</a></td>
</tr>";
}
echo "</table>";
}
else{
  echo "<font style='color:red; font-size:20px;'>no and comment of feedback!</font>";
}
