...
<form action="searched.php" method="post"><tr><td></td> <td><input type="text" name="serial" class="form-control" style="width: 400px; font-size: 20px; font-family: comic sans ms; border: 3px solid black; border-radius: 5px;" placeholder="search prisoner using serial Number"></td> <td><input type="submit" name="search" value="search" class="btn btn-info form-control"></td> </tr></form>
    ...
    <tr style="text-align: center;"><td style="font-size: 20px; background-color: LightSteelBlue; color: white; font-family: comic sans Ms;">Do you know about Assosa Zone</td>
        ...
        <tr><td class="text-justify" style="font-size: 14px; width: 300px;">  Assosa Zone  Benshangul/Gumuz, is one of the 11 ethnic divisions (kililoch) of Ethiopia. It was previously known as Region 6.The region's capital is Assosa<br>
            <img src="../message.jfif" width="100%"></td> 
            <td style="width: 450px;" class="text-justify" rowspan="3">
          
<?php 
$sql="select *from inspector_message where status=0 and zone='".$zone."'";
$query=mysqli_query($con,$sql);
$report=mysqli_num_rows($query);
if ($report>0) {
  echo "<div style='width:400px; overflow:auto;'><table class='table table-bordered'><tr><th>Date</th> <th>Sender</th> <th>message</th>  <th>status</th></tr>";
 while ($row=mysqli_fetch_array($query)) {
   echo "<tr style='color:black;font-size:20px;'><td>".$row['date']."</td> <td>".$row['sender']."</td> <td>".$row['message']."</td> <td><a href='read.php?id=".$row['id']."'>read</a></td></tr>";
 }
 echo "</div></table>";
}
else{
  echo "<font style='color:red; font-size:20px;'>no message!</font>";
}
